...
function getRandomPrice() {
    return Math.random() * (50000 - 1000) + 1000; // Harga antara $1,000 dan $50,000
}
...
const jimp = require('jimp'); // Tambahkan ini di awal file

async function createCollage(images) {
    const gridSize = Math.ceil(Math.sqrt(images.length));
    const imageSize = 150;
    const collageSize = gridSize * imageSize;

    const collage = new jimp(collageSize, collageSize, '#FFFFFF'); // Latar belakang putih
    let x = 0, y = 0;
    ...
    for (let i = 0; i < images.length; i++) {
        try {
            ...
            collage.composite(image, x * imageSize, y * imageSize);
            ...
            console.error(`Gagal memuat gambar: ${images[i]}`);
        }
    }
...
module.exports = { unixTimestampSeconds, generateMessageTag, processTime, webApi, getRandom, getBuffer, fetchJson, runtime, clockString, sleep, isUrl, getTime, formatDate, tanggal, formatp, jsonformat, reSize, toHD, logic, generateProfilePicture, bytesToSize, checkBandwidth, getSizeMedia, parseMention, getGroupAdmins, readFileTxt, readFileJson, isCommandAvailable, getHashedPassword, generateAuthToken, cekMenfes, generateToken, batasiTeks, randomText, getRandomPrice, isEmoji, getTypeUrlMedia, pickRandom, createCollage, getAllHTML };
